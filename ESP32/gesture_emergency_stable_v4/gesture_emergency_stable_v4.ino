...
constexpr bool ENABLE_APDS_REFERENCE = false;
...
constexpr uint16_t SERVO_HOLD_TIME_MS = 300;
...
constexpr uint32_t BUTTON_PRESS_STABLE_MS = 30;
...
constexpr int32_t MIN_ACCEL_PEAK_TO_PEAK = 2500;
constexpr int32_t MIN_GYRO_PEAK_TO_PEAK = 2500;
constexpr int32_t MIN_ACCEL_ABSOLUTE_SUM = 2000;
constexpr uint8_t MAX_IDENTICAL_SAMPLE_RUN = 4;
...
int moveServoForGesture(const String &gesture)
{
  ...
  // Keep the servo electrically quiet between gestures. Continuous PWM
  // can inject ground noise into the sensor wiring even with external 5 V.
  if (!gestureServo.attached())
  {
    gestureServo.attach(
      SERVO_PIN,
      544,
  ...
  {
    Serial.println(
      "WARNING: Servo could not be attached for this gesture."
    );
...
int32_t absoluteSampleValue(int16_t value)
{
  const int32_t expandedValue =
    static_cast<int32_t>(value);

  return expandedValue < 0
    ? -expandedValue
    : expandedValue;
}

bool isMpuSamplePlausible(const MotionSample &sample)
{
  const bool allChannelsZero =
    sample.ax == 0 &&
    sample.ay == 0 &&
    sample.az == 0 &&
    sample.gx == 0 &&
  ...
  const int32_t accelerationAbsoluteSum =
    absoluteSampleValue(sample.ax) +
    absoluteSampleValue(sample.ay) +
    absoluteSampleValue(sample.az);

  return accelerationAbsoluteSum >=
    MIN_ACCEL_ABSOLUTE_SUM;
}

bool motionSamplesAreIdentical(
  const MotionSample &first,
  const MotionSample &second
)
{
  return
    first.ax == second.ax &&
...
bool readMpuSampleOnce(MotionSample &sample)
{
  ...
  return isMpuSamplePlausible(sample);
}
...
void recoverI2cController()
{
  ...
  // Release a slave that may be holding SDA low by clocking SCL manually.
  ...
  // Generate an I2C STOP condition before restarting the controller.
...
bool ensureMpuReadyForCapture()
{
  uint8_t deviceId = 0;

  for (uint8_t check = 0; check < 2; check++)
  {
    if (
      readMpuRegister(MPU6050_WHO_AM_I, deviceId) &&
      deviceId != 0x00 &&
      deviceId != 0xFF
    )
    {
  ...
  Serial.println(
    "MPU6050 pre-capture check failed. Recovering I2C bus and sensor..."
  );
  ...
  for (uint8_t attempt = 1; attempt <= 3; attempt++)
  {
    ...
    {
      Serial.printf(
        "MPU6050 recovery successful on attempt %u.\n",
  ...
  Serial.println(
    "ERROR: MPU6050 recovery failed before capture."
  );
...
bool printAndValidatePeakToPeakValues()
{
  ...
  Serial.printf(
    "Motion thresholds: accel >= %ld AND gyro >= %ld\n",
...
void runCapture()
{
  ...
  )
  {
    ...
    {
      identicalSampleRun = 0;
    }

    if (
      identicalSampleRun >=
      MAX_IDENTICAL_SAMPLE_RUN
    )
    {
      digitalWrite(LED_PIN, LOW);

      Serial.printf(
        "ERROR: MPU6050 data froze at sample %u. "
        "Repeated identical samples were detected.\n",
        static_cast<unsigned int>(sampleIndex)
      );
...
void setup()
{
  ...
  {
    Serial.println(
      "APDS-9960 reference disabled in final inference mode."
    );
  }
  ...
  Serial.println(
    ENABLE_APDS_REFERENCE
      ...
      : "MPU6050 initialized. APDS reference is disabled."
  );
...
void loop()
{
  ...
  // A real button press must remain physically LOW. This rejects short
  // electrical spikes that can trigger the interrupt without a human press.
  if (digitalRead(BUTTON_PIN) != LOW)
  {
    return;
  }

  delay(BUTTON_PRESS_STABLE_MS);
